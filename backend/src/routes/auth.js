...
authRouter.post("/login", async (req, res) => {
  try {
    ...
    const user = await User.findOne({ emailId: normalizedEmail }).setOptions({ includeDeleted: true });
...
// 4. EDIT PROFILE (name / career goal / semester only — branch is
// set exactly once via PATCH /profile/branch, never here)
authRouter.patch("/profile/edit", userAuth, async (req, res) => {
  try {
    const { firstName, lastName, careerGoal, currentSemester } = req.body;

    if (!firstName || !careerGoal) {
      return res.status(400).json({ success: false, message: "First name and career goal are required." });
    }
    ...
    const updatedUser = await User.findByIdAndUpdate(
      req.user._id,
      {
        $set: {
          ...
          lastName: lastName || "",
          careerGoal,
          currentSemester: currentSemester || req.user.currentSemester,
        },
      },
      { new: true, runValidators: true }
    );

    res.json({ success: true, message: "Profile updated successfully.", user: publicUser(updatedUser) });
  } catch (err) {
    res.status(400).json({ success: false, message: err.message });
  }
});
...
authRouter.patch("/profile/branch", userAuth, async (req, res) => {
  try {
    ...
    if (!User.BRANCHES.includes(branch)) {
      return res.status(400).json({ success: false, message: `"${branch}" is not a recognized branch code.` });
    }
    ...
    res.json({ success: true, message: "Branch confirmed.", user: publicUser(req.user) });
  } catch (err) {
    res.status(400).json({ success: false, message: err.message });
  }
});
    ...
    try {
      ...
      if (!req.user.branch) {
        return res.status(400).json({
          success: false,
          message:
            "Select your branch before choosing a career goal.",
        });
      }
      ...
      branches.forEach((branch) => {
        (branch.career_paths || []).forEach((path) => {
          if (path.career) availableCareerOptions.set(path.career.toLowerCase(), path.career);
        });
      ...
      const canonicalCareerGoal = availableCareerOptions.get(cleanCareerGoal.toLowerCase());
      if (!canonicalCareerGoal) {
        return res.status(400).json({
          success: false,
          message:
            `"${cleanCareerGoal}" is not a recognized career goal.`,
        });
      }
      ...
      return res.status(200).json({
        success: true,
        message:
          "Career goal and specialization saved successfully.",
        user: publicUser(req.user),
      });
    } catch (err) {
      console.error(
...
authRouter.delete("/profile/delete", userAuth, async (req, res) => {
  try {
    ...
    res
      ...
      .json({ success: true, message: "Account deactivated. You can restore it within 7 days." });
  } catch (err) {
    res.status(400).json({ success: false, message: err.message });
  }
});
...
authRouter.post("/restore-account", async (req, res) => {
  try {
    ...
    if (!user.isDeleted) return res.status(400).json({ success: false, message: "Account is already active." });
